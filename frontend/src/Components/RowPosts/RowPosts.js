...
// import { AppContext } from "../../AppContext"; // using AppContext
import { useSelector } from "react-redux";

function RowPosts(props) {
  // const {user} = useContext(AppContext) // using AppContext
  ...
  function mouseEnter(id) {
    let data = movie.find((value) => value.id === id);
    setMouseEnterData(data);
  }

  /*Row Post Click Handler */
  const postClickHandler = (id) => {
    if (user) navigate(`/movie/${id}`);
    else {
      ...
      <div className="posters">
        {movie.map((obj) => (
          <div key={obj.id} className="image-wrapper">
            <img
              onMouseEnter={() => mouseEnter(obj.id)}
              onMouseLeave={() => setMouseEnterData()}
              className={props.isSmall ? "smallPoster" : "poster"}
              src={`${IMAGE_URL + obj.backdrop_path}`}
              alt="poster"
              onClick={() => {
                postClickHandler(obj.id);
              }}
            />

            <div className="poster-details">
              <h3 className="poster-title">
                {(mouseEnterData && mouseEnterData.original_title) ||
                  (mouseEnterData && props.isSmall && mouseEnterData.title)}
              </h3>
              <h4 className="avg-vote">
                {mouseEnterData ? mouseEnterData.vote_average : ""}
                {star}
              </h4>
              <button
                onClick={() => {
